...
function useAutoScroll(contentRef: React.RefObject<HTMLDivElement | null>, content: string) {
  useEffect(() => {
...
function ThinkingHeader({ timer, message = "Moorcheh AI is thinking..." }: ThinkingHeaderProps) {
  return (
...
function ThinkingContent({ contentRef, content }: ThinkingContentProps) {
  return (
    <div
      ...
      aria-live="polite"
      className="h-full overflow-y-auto overflow-x-hidden p-4 text-secondary-foreground scrollbar-hide"
      ref={contentRef}
      role="log"
      style={{ 
...
export default function AIThinking({ 
  className, 
  spinner = true, 
  message = "Moorcheh AI is thinking...",
  content 
}: AIThinkingProps) {
  const contentRef = useRef<HTMLDivElement>(null);
  const timer = useTimer();
  const displayContent = content || THINKING_CONTENT;
  
  useAutoScroll(contentRef, displayContent);

  return (
    <div className={cn("flex max-w-xl flex-col gap-4", className)}>
      {spinner && <ThinkingHeader timer={timer} message={message} />}
      {!spinner && message && (
        <div className="flex items-center gap-2">
          <span className="relative inline-block text-sm">{message}</span>
        </div>
      )}
      <ContentCard content={displayContent} contentRef={contentRef} />
      <ShimmerStyles />
    </div>
  );
}
